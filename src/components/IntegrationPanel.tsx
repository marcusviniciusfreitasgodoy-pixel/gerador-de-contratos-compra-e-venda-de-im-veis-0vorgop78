...
export function IntegrationPanel() {
  ...
  const handleSave = async () => {
    ...
    const cleanedKeys = {
      anthropic: keys.anthropic.replace(/[^\x21-\x7E]/g, ''),
      openai: keys.openai.replace(/[^\x21-\x7E]/g, ''),
      gemini: keys.gemini.replace(/[^\x21-\x7E]/g, ''),
    }
    setKeys(cleanedKeys)
    ...
    try {
      await pb.collection('users').update(user.id, {
        anthropic_api_key: cleanedKeys.anthropic,
        openai_api_key: cleanedKeys.openai,
        gemini_api_key: cleanedKeys.gemini,
      })
