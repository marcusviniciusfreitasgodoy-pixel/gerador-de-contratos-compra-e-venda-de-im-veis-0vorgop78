...
onRecordAfterUpdateSuccess(async (e) => {
  ...
  const originalFileName = e.record.original().getString('source_file')

  if (!fileName || fileName === originalFileName) return e.next()
  ...
  try {
    ...
    $app.logger().error('File extraction update error', 'record', recId, 'error', err.message)
  }
